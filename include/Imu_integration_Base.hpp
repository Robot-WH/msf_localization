...
//   }
     ...
     * 
     */
    void predict(Eigen::Vector3d const& angular_velocity, Eigen::Vector3d const& linear_acceleration, Eigen::Vector3d &tmp_P, 
                        Eigen::Vector3d &tmp_V, Eigen::Quaterniond &tmp_Q, Eigen::Vector3d &tmp_Ba, Eigen::Vector3d &tmp_Bg, double const& dt)
    {
        // 中值积分           
        Eigen::Vector3d un_acc_0 = tmp_Q * (acc_0 - tmp_Ba) - G;                // 上一刻加速度  
        Eigen::Vector3d un_gyr = 0.5 * (gyr_0 + angular_velocity) - tmp_Bg;
        // 更新预测旋转Q  
        tmp_Q = tmp_Q * Utility::deltaQ(un_gyr * dt);                                                           // 更新旋转
        Eigen::Vector3d un_acc_1 = tmp_Q * (linear_acceleration - tmp_Ba) - G;      // 当前加速度  
  ...
  private:
    Eigen::Vector3d G = {0.0, 0.0, 9.8};                                   // 重力的值  即静止时IMU的测量值 

    double dt;
    Eigen::Vector3d acc_0 = {0., 0., 0.}, gyr_0 = {0., 0., 0.};            // 上一时刻的测量
    Eigen::Vector3d acc_1 = {0., 0., 0.}, gyr_1 = {0., 0., 0.};            // 当前时刻的测量 
